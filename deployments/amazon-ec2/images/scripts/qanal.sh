...
exec /usr/bin/docker run --name qanal \
       ...
       -e 'KAFKA_TOPICS_SPEC={ :topic "events" :partitions :all :type :plain :indexing {:strategy :simple :index "events" :doc-type "events" :timestamp-field "timestamp" :timestamp-field-format :millis :id-field "id"}}' \
