(ns samsara.qanal.coordinator.coordinator-zookeeper
...
(defn distribute
  ...
  There's probably a simpler/more elegant way of doing this
  "
  [bucket-num data]
  (loop [splits (split-at bucket-num data)
         buckets (vec (repeat bucket-num []))]
    (if (empty? (first splits))
      buckets
      (let [taken (first splits)
            rest (second splits)
            bucket-m {:curr-pos 0 :buckets buckets}
            reduce-fn (fn [{:keys [curr-pos buckets]} s]
                        {:curr-pos (inc curr-pos) :buckets (update buckets curr-pos conj s)})
            updated-m (reduce reduce-fn bucket-m taken)]
        (recur (split-at bucket-num rest) (:buckets updated-m))))))

;; (defn distribute2 [bucket-num data]
;;   (let [bucket-keys (cycle (range 0 bucket-num))
;;         data-bucket-seq (interleave bucket-keys data)]
;;     (loop [splits (split-at 2 data-bucket-seq)
;;            buckets (vec (repeat bucket-num []))]
;;       (if (empty? (first splits))
;;         buckets
;;         (let [taken (first splits)
;;               rest (second splits)
;;               bckt-pos (first taken)
;;               to-add (second taken)]
;;           (recur (split-at 2 rest) (update buckets bckt-pos conj to-add)))))))
      ...
      (loop [new-jobs  (.all-jobs worker)
        ...
        (let [qanal-ids (mapv (fn [i] (.getId i)) instances)]
          ...
          (recur (.all-jobs worker)
                 ...
                 assigned-jobs)))
      (catch Exception e
        ;; catching and throwing because curator will internally quietly handle
        ;; these exceptions, thus it is hard to know when our code is causing exceptions
        ;; Note: The thread that runs this function will be interrupted when leadership
        ;; is lost.
        (log/error "Lost Leadership. Participation-id:" p-id " Due to" (.getMessage e))
        (throw e)))))
...
(deftype ZookeeperCoordinator [id zk-connect worker mutable-state]
  Coordinator

  (start [this]
    (locking mutable-state
      (when-not (:running @mutable-state)
        (reset! mutable-state (complete-startup id zk-connect worker)))))

  (stop [this]
    (locking mutable-state
      (when (:running @mutable-state)
        (swap! mutable-state complete-shutdown worker)))))
